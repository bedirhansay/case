...
import { StoreContext, ContextProps } from "@/lib/context/ContextProvider";
...
describe("render Hook", () => {
  test("useContexData should return context value", () => {
    const { result } = renderHook(() => useContexData(), {
      wrapper: ({ children }) => (
        <StoreContext.Provider value={mockContextData}>
          {children}
